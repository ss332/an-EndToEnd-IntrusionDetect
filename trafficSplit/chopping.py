...
def tiks(sequence, c1, c2):
    ...
    for i in range(100000):
        ...
        if time[0] <= flows['time'][i] <= time[1] and protocol == 6 and dst_port == 8080:
            match_time = 1
        if time[2] <= flows['time'][i] <= time[3] and protocol == 6 and dst_port == 8080:
            match_time = 2
        ...
        if match_s_ip == 1 and match_d_ip == 1:
            if match_time == 1:
                labels.append(c1)
            elif match_time == 2:
                labels.append(c2)
...
tiks(8, 14, 14)
flows = pd.read_csv('../files/f_process8.csv')
print(flows[flows['label'] == 0].shape)
print(flows[flows['label'] == 14].shape)
print(flows[flows['label'] == 14].shape)
